...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (!valid_name($name)) {
    $nameError = "Please enter a valid name (only letters, min 3 chars).";
    $isValid = false;
  }
  ...
  if ($isValid) {
    ...
    // ✅ Designer needs approval (is_approved=0), users are auto-approved (is_approved=1)
    $isApproved = ($role === 'designer') ? 0 : 1;

    $stmt = $conn->prepare("INSERT INTO users(name,email,password,role,is_approved) VALUES(?,?,?,?,?)");

    try {
      $stmt->execute([$name, $email, $hash, $role, $isApproved]);

      if ($role === 'designer') {
        echo "<script>alert('Signup successful! Please wait for admin approval before logging in.'); location.href='login.php';</script>";
      } else {
        echo "<script>alert('Signup successful! You can now log in.'); location.href='login.php';</script>";
      }
      exit;
...
<head>
  ...
  <title>Signup | ELEGANCE SALONE</title>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css"/>
  <link href="https://fonts.googleapis.com/css2?family=Poppins&display=swap" rel="stylesheet">
  <style>
  :root {
  --brand-accent: #d4a373; /* DecorVista luxury gold */
...
<body>
  <form class="form-wrapper" method="POST" autocomplete="off" novalidate>
    <h2>Signup</h2>
    <div class="subtitle">JOIN OUR ELEGANCE SALONE COMMUNITY</div>
    ...
    <div class="form-group">
      ...
      <?php if ($nameError): ?>
        <div class="error"><?= $nameError ?></div><?php endif; ?>
    </div>
    ...
    <button type="submit" class="btn" name="signup">SIGN UP <i class="fa fa-arrow-right"></i></button>
    <div class="bottom-text">Already have an account? <a href="login.php">SIGN IN</a></div>
  </form>
</body>
</html>
